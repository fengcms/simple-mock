...
*/
export const mockPageListData = (obj: Record<string, any>, page = 1, pageSize = 10) => {
  const list = []
  // 根据参数计算最大最小 index 值
  ...
  if (page <= 5) {
    const max = page === 5 ? Number.parseInt(String(maxIndex - pageSize / 2)) : maxIndex
    for (let i = minIndex; i <= max; i++) {
      const objStr = JSON.stringify(obj).replace(/#INDEX#/g, String(i))
